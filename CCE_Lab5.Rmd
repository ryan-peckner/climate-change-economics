...
GrossOutputWithIncomeElasticity = function(t,temp_increase,abatement_cost,income_elasticity) {
  if (t == 1) {
    ...
    first_term = (1-delta)*(Y_t_1/(forcings4$total.factor.productivity[t-1]*(forcings2$population[t-1]^(1-alpha1))))^(1/alpha1)
    second_term  = s*(1 - abatement_cost[t-1] - Psi*(temp_increase^2)*((Y_t_1/Y_0)^(income_elasticity)))*Y_t_1
    return(forcings4$total.factor.productivity[t]*((first_term + second_term)^(alpha1))*(forcings2$population[t]^(1-alpha1)))
  }
}
...
for (t in 2:nrow(forcings5)) {
  ...
  forcings5$temp.increase_0elasticity[t] = 
          GlobalAvgTempIncrease_FromEmissions(t,
                                    emissions = 
                                     rep(KayaEmissionsFromGDP(t,population = forcings2$population,
